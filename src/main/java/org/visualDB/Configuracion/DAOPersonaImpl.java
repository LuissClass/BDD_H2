package org.visualDB.Configuracion;
...
public class DAOPersonaImpl implements DAOPersona {
    ...
    public List<Persona> obtenerTodasPersonasPorRangoEdad(int min, int max) {
        List<Persona> personas = new ArrayList<>();
        try {
            PreparedStatement ps = conexion.prepareStatement("SELECT * FROM BDPRUEBA1.TABLA1PRUEBA WHERE numero>=? and numero<=?");
            ps.setInt(1, min);
            ps.setInt(2, max);
            ResultSet personaAux = ps.executeQuery();
            while (personaAux.next()) {
